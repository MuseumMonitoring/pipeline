    ...
    async init(this: Args & this): Promise<void> {
        ...
        http.createServer(async (request, response) => {
            ...
            try {
                ...
                for (const p of this.publicGroups) {
                    if (identifier.includes("group-" + p)) {
                        await new Promise(res => response.write(publicString(), res));
                    }
                }
                ...
                if (match) {
                    ...
                    const users = this.foundUsers.filter(u => allowedGroups.has(u.group_id));
                    let c = 0;
                    for (const u of users) {
                        await new Promise(res => response.write(emailAuth(u.email, c++), res));
                    }
