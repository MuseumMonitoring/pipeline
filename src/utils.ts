...
export class Logger extends Processor<LoggerArgs> {
    async startReader(reader: Reader) {
        this.logger.info("Starting with reader " + reader.uri)
        for await (const msg of reader.strings()) {
            this.logger.info(reader.uri + "\n" + msg);
        }
    }
    async init(this: LoggerArgs & this): Promise<void> {
    }
    async transform(this: LoggerArgs & this): Promise<void> {
        this.logger.info("Transform logger with " + this.streams.length + " readers");
        await Promise.allSettled(
            this.streams.map(r => this.startReader(r))
        )
    }
    async produce(this: LoggerArgs & this): Promise<void> {
    }
