...
class Fetched {
  ...
  public static async fetch(url: string): Promise<Fetched> {
    ...
    try {
      ...
      return await Fetched.fetch(url);
    }
  }
  ...
  nextUrl(): string | undefined {
    ...
    if (next) {
      const url_url = new URL(next.url, this.url);
      return url_url.href;
    }
  }
...
export class Fetcher extends Processor<FetchArgs> {
  ...
  async init(this: FetchArgs & this): Promise<void> {
    ...
    if (this.save_path) {
      try {
        const { url, at }: { url: string; at: number } = JSON.parse(
          readFileSync(this.save_path, { encoding: "utf8" }),
        );
        start = url;
        this.at = at;
      } catch (ex: any) {
    ...
    if (this.delayed) {
      this.waitFor = (async () => {
        for await (const v of this.delayed!.anys()) {
          this.logger.error(
            "Didn't expect a message, but got " + JSON.stringify(v),
          );
        }
      })();
    } else {
  ...
  save(this: FetchArgs & this) {
    if (this.save_path) {
      ...
      writeFileSync(this.save_path, JSON.stringify(state), {
        encoding: "utf8",
      });
    }
  }
