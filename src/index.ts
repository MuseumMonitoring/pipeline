...
import { CONTENT_TYPE, guardedStreamFrom, INTERNAL_QUADS, RepresentationMetadata, type DataAccessor, type Guarded, type Representation, type ResourceIdentifier } from "@solid/community-server";
...
export class ProxyDataAccessor implements DataAccessor {
    ...
    async writeDocument(identifier: ResourceIdentifier, data: Guarded<Readable>, metadata: RepresentationMetadata): Promise<void> { throw "nah"; }
    async writeContainer(identifier: ResourceIdentifier, metadata: RepresentationMetadata): Promise<void> { throw "nah"; }
    async writeMetadata(identifier: ResourceIdentifier, metadata: RepresentationMetadata): Promise<void> { throw "nah"; }
    async deleteResource(identifier: ResourceIdentifier): Promise<void> { throw "nah"; }
}
